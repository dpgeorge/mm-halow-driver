...
#ifndef MM_HALOW_DEBUG
#define MM_HALOW_DEBUG (0)
#endif

#if MM_HALOW_DEBUG
#define debug_printf(...) MM_HALOW_PRINTF(__VA_ARGS__)
#else
#define debug_printf(...)
#endif
...
#define MM_HALOW_PS_TIMEOUT_DEFAULT_MS (100)

mm_halow_t mm_halow_state = {
    #if MM_HALOW_ENABLE_AP
    .ap_auth = MM_HALOW_SEC_SAE,
    #endif
    .ps_timeout_ms = MM_HALOW_PS_TIMEOUT_DEFAULT_MS,
...
static bool mm_halow_scan_idle_cond(void *arg);
...
#define MM_HALOW_TX_BUF_SIZE (MM_HALOW_MTU + SIZEOF_ETH_HDR)
...
#define MM_HALOW_TX_READY_MS (250)
static uint8_t *mm_halow_tx_buf;
...
static int mm_halow_status_to_errno(enum mmwlan_status status) {
    switch (status) {
        case MMWLAN_SUCCESS:
            return 0;
        ...
        case MMWLAN_UNAVAILABLE:
            return -MM_HALOW_EAGAIN;
        case MMWLAN_NO_MEM:
            return -MM_HALOW_ENOMEM;
        case MMWLAN_TIMED_OUT:
            return -MM_HALOW_ETIMEDOUT;
        case MMWLAN_CHANNEL_LIST_NOT_SET:
            return -MM_HALOW_ENODEV;
        case MMWLAN_CHANNEL_INVALID:
            return -MM_HALOW_ERANGE;
        case MMWLAN_NOT_FOUND:
            return -MM_HALOW_ENOENT;
        case MMWLAN_NOT_SUPPORTED:
            return -MM_HALOW_EOPNOTSUPP;
        case MMWLAN_VIF_ERROR:
            return -MM_HALOW_ENXIO;
        default:
            return -MM_HALOW_EIO;
    }
}
...
static void mm_halow_sta_status_cb(enum mmwlan_sta_state sta_state) {
...
static void mm_halow_link_state_cb(enum mmwlan_link_state link_state, void *arg) {
    ...
    if (link_state == MMWLAN_LINK_UP) {
        mm_halow_cb_tcpip_set_link_up(self, MM_HALOW_ITF_STA);
    } else {
        mm_halow_cb_tcpip_set_link_down(self, MM_HALOW_ITF_STA);
    }
}
...
static void mm_halow_rx_cb(uint8_t *header, unsigned header_len,
    ...
    mm_halow_t *self = arg;
    mm_halow_cb_process_ethernet(self, MM_HALOW_ITF_STA, header, header_len, payload, payload_len);
}
...
int mm_halow_init(mm_halow_t *self) {
    ...
    if (!mm_halow_osal_init()) {
        return -MM_HALOW_ENOMEM;
    }
    mm_halow_tx_buf = mm_halow_osal_malloc(MM_HALOW_TX_BUF_SIZE);
    if (mm_halow_tx_buf == NULL) {
    ...
    mm_halow_apply_pm(self);
    mm_halow_apply_radio(self);

    mmwlan_register_fatal_error_handler(mm_halow_fatal_error_cb, self);
    mmwlan_register_link_state_cb(mm_halow_link_state_cb, self);
    mmwlan_register_rx_cb(mm_halow_rx_cb, self);

    mmwlan_get_vif_mac_addr(MMWLAN_VIF_STA, self->mac);

    self->initted = true;
    mm_halow_poll = mm_halow_poll_func;
    return 0;
}
...
#define MM_HALOW_SCAN_DWELL_MS (110)
...
static int mm_halow_set_country(mm_halow_t *self, const char *country) {
    ...
    return mm_halow_status_to_errno(mmwlan_set_channel_list(channels));
}

#if MM_HALOW_ENABLE_AP
static void mm_halow_ap_sta_status_cb(const struct mmwlan_ap_sta_status *status, void *arg) {
    mm_halow_t *self = (mm_halow_t *)arg;
    ...
    if (status->state == MMWLAN_AP_STA_AUTHORIZED && self->ap_sta_count < MM_HALOW_AP_MAX_STAS) {
...
static const struct mmwlan_s1g_channel *mm_halow_ap_channel(mm_halow_t *self) {
...
static int mm_halow_ap_enable(mm_halow_t *self) {
    if (self->pm != MM_HALOW_PM_NONE) {
        return -MM_HALOW_EPERM;
    }
    ...
    self->ap_sta_count = 0;
    args.sta_status_cb = mm_halow_ap_sta_status_cb;
    ...
    if (self->ap_auth == MM_HALOW_SEC_SAE) {
    ...
    if (self->ap_auth == MM_HALOW_SEC_OPEN) {
    ...
    return mm_halow_status_to_errno(mmwlan_ap_enable(&args));
}
...
int mm_halow_wifi_set_up(mm_halow_t *self, int itf, bool up, const char *country) {
    ...
    if (up) {
        ...
        int ret = mm_halow_set_country(self, country);
        if (ret != 0) {
            return ret;
        }
        ...
        if (!self->booted) {
            ...
            ret = mm_halow_status_to_errno(mmwlan_boot(&boot_args));
            if (ret != 0) {
                return ret;
            }
            self->booted = true;
        ...
        if (!(self->itf_state & (1 << itf))) {
            mm_halow_cb_tcpip_init(self, itf);
            self->itf_state |= 1 << itf;
        }
        #if MM_HALOW_ENABLE_AP
        if (itf == MM_HALOW_ITF_AP) {
            mm_halow_cb_tcpip_set_link_up(self, itf);
        }
        #endif
    } else {
        #if MM_HALOW_ENABLE_AP
        ...
        if (self->itf_state & (1 << itf)) {
            mm_halow_cb_tcpip_set_link_down(self, itf);
            mm_halow_cb_tcpip_deinit(self, itf);
            self->itf_state &= ~(1 << itf);
        }
        if (self->itf_state == 0) {
            mm_halow_sched_teardown = true;
...
static void mm_halow_scan_cache_add(mm_halow_t *self, const mm_halow_ev_scan_result_t *res) {
...
size_t mm_halow_wifi_scan_cached(mm_halow_t *self, mm_halow_ev_scan_result_t *out, size_t max) {
    ...
    if (self->link_status != MM_HALOW_LINK_DOWN) {
        return 0;
    }

    if (self->scan_cache == NULL) {
        self->scan_cache = mm_halow_osal_malloc(
            MM_HALOW_SCAN_CACHE_MAX * sizeof(*self->scan_cache));
        if (self->scan_cache == NULL) {
            return 0;
        }
        self->scan_cache_max = MM_HALOW_SCAN_CACHE_MAX;
    }

    if (!self->scan_active) {
        self->scan_cache_len = 0;
        if (mm_halow_scan_start_locked(self) != 0) {
            return 0;
        }
    }
    mm_halow_sched_wait(mm_halow_scan_idle_cond, self, MM_HALOW_SCAN_MS);
...
static uint8_t mm_halow_freq_to_chan(mm_halow_t *self, uint32_t freq_hz, uint8_t bw_mhz) {
    const struct mmwlan_s1g_channel_list *list = self->channels;
...
static void mm_halow_scan_rx_cb(const struct mmwlan_scan_result *result, void *arg) {
    mm_halow_t *self = arg;
    mm_halow_ev_scan_result_t res = { 0 };
    ...
    res.chan_num = mm_halow_freq_to_chan(self, result->channel_freq_hz, result->bw_mhz);
    ...
    res.security = (result->capability_info & MM_HALOW_CAP_PRIVACY) ? MM_HALOW_SEC_SAE : MM_HALOW_SEC_OPEN;

    mm_halow_scan_cache_add(self, &res);
}

// An aborted sweep stops at the end of the channel it is on, so this only has to
// cover one channel dwell, not a whole sweep.
#define MM_HALOW_SCAN_ABORT_MS (5000)

static bool mm_halow_scan_idle_cond(void *arg) {
    return !((mm_halow_t *)arg)->scan_active;
}

static void mm_halow_scan_complete_cb(enum mmwlan_scan_state scan_state, void *arg) {
    mm_halow_t *self = arg;
    self->scan_active = false;
    debug_printf("halow: sweep took %ums, %u results\n",
        (unsigned int)(mm_halow_ticks_ms() - self->scan_started_ms),
...
static int mm_halow_scan_start_locked(mm_halow_t *self) {
    ...
    req.scan_rx_cb = mm_halow_scan_rx_cb;
    req.scan_complete_cb = mm_halow_scan_complete_cb;
    req.scan_cb_arg = self;
    req.args.dwell_time_ms = MM_HALOW_SCAN_DWELL_MS;

    self->scan_active = true;
    self->scan_started_ms = mm_halow_ticks_ms();
...
int mm_halow_wifi_join(mm_halow_t *self, size_t ssid_len, const uint8_t *ssid,
    ...
    if (auth_type == MM_HALOW_SEC_SAE && key_len == 0) {
        return -MM_HALOW_EINVAL;
    }
    ...
    if (auth_type == MM_HALOW_SEC_OPEN) {
        args.pmf_mode = MMWLAN_PMF_DISABLED;
    }
    if (bssid != NULL) {
    ...
    if (self->scan_active) {
        mmwlan_scan_abort();
        mm_halow_sched_wait(mm_halow_scan_idle_cond, self, MM_HALOW_SCAN_ABORT_MS);
    }
...
int mm_halow_wifi_leave(mm_halow_t *self, int itf) {
    ...
    self->link_status = MM_HALOW_LINK_DOWN;
    return mm_halow_status_to_errno(mmwlan_sta_disable());
}

int mm_halow_wifi_link_status(mm_halow_t *self, int itf) {
    if (itf == MM_HALOW_ITF_AP) {
        return (self->itf_state & (1 << MM_HALOW_ITF_AP)) ? MM_HALOW_LINK_UP : MM_HALOW_LINK_DOWN;
    }
    return self->link_status;
}

int mm_halow_tcpip_link_status(mm_halow_t *self, int itf) {
    int status = mm_halow_wifi_link_status(self, itf);
...
int mm_halow_wifi_get_mac(mm_halow_t *self, int itf, uint8_t mac[6]) {
    enum mmwlan_vif vif = (itf == MM_HALOW_ITF_AP) ? MMWLAN_VIF_AP : MMWLAN_VIF_STA;
...
int mm_halow_wifi_get_bssid(mm_halow_t *self, uint8_t bssid[6]) {
    (void)self;
    return mm_halow_status_to_errno(mmwlan_get_bssid(bssid));
}
...
static int mm_halow_apply_pm(mm_halow_t *self) {
    enum mmwlan_status status = mmwlan_set_power_save_mode(
        self->pm == MM_HALOW_PM_NONE ? MMWLAN_PS_DISABLED : MMWLAN_PS_ENABLED);
    if (status != MMWLAN_SUCCESS) {
        return mm_halow_status_to_errno(status);
    }
...
int mm_halow_wifi_pm(mm_halow_t *self, uint32_t pm) {
    #if MM_HALOW_ENABLE_AP
    if (pm != MM_HALOW_PM_NONE && (self->itf_state & (1 << MM_HALOW_ITF_AP))) {
    ...
    self->pm = pm;
    // Only reaches morselib once it has been initialised; mm_halow_init() applies
    // whatever was configured before that.
    if (self->booted) {
        int ret = mm_halow_apply_pm(self);
        if (ret != 0) {
            self->pm = prev;
            return ret;
        }
...
int mm_halow_wifi_fixed_rate(mm_halow_t *self, int mcs, int bw_mhz, int gi) {
    ...
    return mm_halow_status_to_errno(mmwlan_ate_override_rate_control(mcs, bw_mhz, gi));
}
...
int mm_halow_wifi_set_radio(mm_halow_t *self, int what, uint32_t value) {
    ...
    switch (what) {
        ...
        case MM_HALOW_RADIO_LISTEN:
            ...
            break;
        case MM_HALOW_RADIO_WNM_PD:
            self->wnm_powerdown = value;
            break;
        case MM_HALOW_RADIO_TXPOWER:
...
int mm_halow_wifi_set_duty_cycle(mm_halow_t *self, int mode) {
    ...
    return mm_halow_status_to_errno(mmwlan_set_duty_cycle_mode(mode));
}
...
int mm_halow_wifi_get_duty_cycle(mm_halow_t *self, struct mmwlan_duty_cycle_stats *stats) {
    ...
    return mm_halow_status_to_errno(mmwlan_get_duty_cycle_stats(stats));
}
...
int mm_halow_wifi_twt(mm_halow_t *self, uint64_t interval_us, uint32_t duration_us, int setup) {
    ...
    return mm_halow_status_to_errno(mmwlan_twt_add_configuration(&args));
}
...
int mm_halow_wifi_set_health_check(mm_halow_t *self, uint32_t min_ms, uint32_t max_ms) {
    ...
    if (self->booted) {
        return mm_halow_status_to_errno(mmwlan_set_health_check_interval(min_ms, max_ms));
    }
    return 0;
}
...
int mm_halow_wifi_get_version(mm_halow_t *self, struct mmwlan_version *version) {
    ...
    return mm_halow_status_to_errno(mmwlan_get_version(version));
}
...
int mm_halow_wifi_wnm_sleep(mm_halow_t *self, bool enable, bool powerdown) {
    ...
    return mm_halow_status_to_errno(mmwlan_set_wnm_sleep_enabled_ext(&args));
}
...
int mm_halow_wifi_ap_get_stas(mm_halow_t *self, int *num_stas, uint8_t *macs) {
    // The list is compacted in place by mm_halow_ap_sta_status_cb() as stations
...
int mm_halow_send_ethernet(mm_halow_t *self, int itf, size_t len, const void *buf, bool is_pbuf) {
    ...
    if (mm_halow_sched_task_current() == NULL) {
        ...
        while (mmwlan_tx_wait_until_ready(0) != MMWLAN_SUCCESS) {
            if ((uint32_t)(mm_halow_ticks_ms() - start) >= MM_HALOW_TX_READY_MS) {
    ...
    if (is_pbuf) {
        ...
        if (p->next == NULL) {
            ...
            pbuf_copy_partial(p, mm_halow_tx_buf, len, 0);
            data = mm_halow_tx_buf;
        }
    } else {
        data = buf;
    }
