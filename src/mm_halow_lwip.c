...
static err_t mm_halow_netif_output(struct netif *netif, struct pbuf *p) {
    mm_halow_t *self = netif->state;
    int itf = netif->name[1] - '0';

    if (self->trace_flags & MM_HALOW_TRACE_ETH_TX) {
        MM_HALOW_PRINTF("halow: [txf] itf=%d len=%u\n", itf, (unsigned int)p->tot_len);
    }

    int ret = mm_halow_send_ethernet(self, itf, p->tot_len, p, true);
    if (ret == -MM_HALOW_EAGAIN) {
        // The transmit queue is still full after the driver has been serviced
        // for MM_HALOW_TX_READY_MS.  Drop the frame, which is what an interface
...
#if LWIP_IGMP
static err_t mm_halow_netif_update_igmp_mac_filter(struct netif *netif, const ip4_addr_t *group,
...
static err_t mm_halow_netif_init(struct netif *netif) {
    mm_halow_t *self = netif->state;
    int itf = netif->name[1] - '0';

    netif->linkoutput = mm_halow_netif_output;
    netif->output = etharp_output;
    #if LWIP_IPV6
    netif->output_ip6 = ethip6_output;
    #endif
    netif->mtu = MM_HALOW_MTU;
    netif->flags = NETIF_FLAG_BROADCAST | NETIF_FLAG_ETHARP | NETIF_FLAG_ETHERNET;

    mm_halow_wifi_get_mac(self, itf, netif->hwaddr);
    netif->hwaddr_len = sizeof(netif->hwaddr);

    #if LWIP_IGMP
    netif->flags |= NETIF_FLAG_IGMP;
    netif_set_igmp_mac_filter(netif, mm_halow_netif_update_igmp_mac_filter);
    #endif
...
void mm_halow_cb_tcpip_init(mm_halow_t *self, int itf) {
    ...
    if (itf == MM_HALOW_ITF_AP) {
        ip_2_ip4(&ipconfig[0])->addr = PP_HTONL(MM_HALOW_AP_ADDRESS);
        ip_2_ip4(&ipconfig[1])->addr = PP_HTONL(MM_HALOW_AP_NETMASK);
        ip_2_ip4(&ipconfig[2])->addr = PP_HTONL(MM_HALOW_AP_ADDRESS);
    } else {
        #if !LWIP_DHCP
        // No client to ask, so come up on the configured address rather than on
        // 0.0.0.0, where the interface would never be usable.
        ip_2_ip4(&ipconfig[0])->addr = PP_HTONL(MM_HALOW_STA_ADDRESS);
        ip_2_ip4(&ipconfig[1])->addr = PP_HTONL(MM_HALOW_STA_NETMASK);
        ip_2_ip4(&ipconfig[2])->addr = PP_HTONL(MM_HALOW_STA_GATEWAY);
        #endif
    }
    #endif
    ...
    netif_add(netif, ip_2_ip4(&ipconfig[0]), ip_2_ip4(&ipconfig[1]), ip_2_ip4(&ipconfig[2]),
        self, mm_halow_netif_init, ethernet_input);
    #elif LWIP_IPV6
    netif_add(netif, self, mm_halow_netif_init, ethernet_input);
    #else
    #error "halow needs either IPv4 or IPv6"
    #endif
    #if LWIP_NETIF_HOSTNAME
    netif_set_hostname(netif, MM_HALOW_HOST_NAME);
    #endif
    ...
    if (itf == MM_HALOW_ITF_STA) {
        ...
        #if MM_HALOW_ENABLE_AP && LWIP_IPV4
...
void mm_halow_cb_tcpip_set_link_down(mm_halow_t *self, int itf) {
    ...
    // Only the link goes down here.  Stopping DHCP would send a
    ...
    // mm_halow_cb_tcpip_deinit() instead.
...
void mm_halow_cb_process_ethernet(void *cb_data, int itf,
    ...
    mm_halow_t *self = cb_data;
    struct netif *netif = &self->netif[itf];
    size_t len = header_len + payload_len;

    if (self->rx_deferred) {
        // The driver is being run from inside lwIP, by the wait in
        // mm_halow_send_ethernet().  Handing it a frame here would re-enter it
        // while it is walking its own lists.
        return;
    }

    if (self->trace_flags & MM_HALOW_TRACE_ETH_RX) {
        MM_HALOW_PRINTF("halow: [rxf] itf=%d len=%u\n", itf, (unsigned int)len);
    }
